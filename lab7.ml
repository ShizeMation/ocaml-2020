      ...
      call (f1 7)
        call (f1 5)
          call (f1 3)
            call (f1 1)
            raise Odd
            pop f1 with exception Odd (without returning control)
          pop f1 with exception Odd (without returning control)
        handles exception Odd and returns -5
      returns -5
    returns -5
  returns -5
...
(* The following two versions of the "closest" function take an
  ...
  For diagrams see attached pdf.

  The second one is more efficient if there exists a leaf in the tree
  that equals the value we're trying to find the closest of.
  Because it would stop searching due to a raised exception and pop
  the stack until the handler, which returns that value.
...
exception IsNegative
let f_except (x:int) =
  try
    if x < 0 then raise IsNegative else x/2 + 1
  with
    | IsNegative -> 0